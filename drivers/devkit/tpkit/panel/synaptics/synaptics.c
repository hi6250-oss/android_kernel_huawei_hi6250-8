...
char synaptics_raw_data_limit_flag;
...
extern void ts_kit_anti_false_touch_param_achieve(struct ts_kit_device_data *chip_data);
extern int ts_oemdata_type_check_legal(u8 type, u8 len);
extern void ts_kit_algo_det_ght_init(void);
...
static int synaptics_parse_dts(struct device_node *device, struct ts_kit_device_data *chip_data);
...
#define BLANC501100_TIANMA_FLAG "huawei,blac501100_tianma_flag"
...
extern bool synaptics_tddi_new_seq(void);
extern void lcd_huawei_ts_synaptics_register(struct tp_synaptics_dev_ops *tp_synaptics_dev_ops);
struct tp_synaptics_dev_ops tpkit_synaptics_ops = {
	.synaptics_tddi_power_seq = synaptics_tddi_new_seq,
};
...
{
	if (rmi4_data->synaptics_chip_data->ts_platform_data->fpga_flag == 1)
		return 0;
...
static void synaptics_chip_touch_switch(void){
	...
	memcpy(in_data, rmi4_data->synaptics_chip_data->touch_switch_info, MAX_STR_LEN);
	...
	*ptr_end = 0;
	...
	if ((TS_SWITCH_TYPE_DOZE != (stype & TS_SWITCH_TYPE_DOZE))&&
		(TS_SWITCH_TYPE_GAME!= (stype & TS_SWITCH_TYPE_GAME))){
		TS_LOG_ERR("stype not DOZE or GAME:%d, invalid\n", stype);
		goto out;
	}
	...
	switch (soper){
		...
		case TS_SWITCH_GAME_ENABLE:
			...
			if (!(value & (1<<rmi4_data->synaptics_chip_data->game_control_bit))){
				value |= (1<<rmi4_data->synaptics_chip_data->game_control_bit);
	...
	if (unlikely
		...
		if (!rmi4_data->synaptics_chip_data->ts_platform_data->get_info_flag) {
			if (SYNAPTICS_S3718 != rmi4_data->synaptics_chip_data->ic_type
				...
				if (retval < 0)
					TS_LOG_ERR("failed to get configid\n");
				synap_fw_data_s3718_init(rmi4_data);
...
int create_and_get_u16_array(struct ts_kit_device_data *chip_data, struct device_node *dev_node, const char *name)
{
	...
	if(NULL == chip_data || NULL == dev_node || NULL == name) {
	...
	for (i = 0; i < sz; i++) {
		chip_data->trx_short_circuit_array[i] = (u16)be32_to_cpup(values++);
	}
...
static void synap_parse_chip_hybrid_specific_dts(struct ts_kit_device_data *chip_data, struct device_node *device)
{
	int retval = 0;

	if(NULL == chip_data || NULL == device) {
		TS_LOG_ERR("%s: chip_data or device is NULL\n", __func__);
		return;
	}

	retval = of_property_read_u32(device, NOT_DELAY_ACT, &chip_data->delay_for_fw_update);
	if (retval) {
		TS_LOG_INFO("%s: delay_for_fw_update has NOT been set\n", __func__);
	}
	TS_LOG_INFO("%s: delay_for_fw_update = %d\n", __func__, chip_data->delay_for_fw_update);

	retval = of_property_read_u32(device, SUPPORT_SHORT_TEST, &chip_data->support_s3320_short_test);
	if (retval) {
		TS_LOG_ERR("%s: support_s3320_short_test has NOT been set\n", __func__);
		goto short_test_unsupport;
	}
	TS_LOG_INFO("%s: support_s3320_short_test = %d\n", __func__, chip_data->support_s3320_short_test);

	retval = create_and_get_u16_array(chip_data, device, TRX_SHORT_CIRCUIT);
	if(retval < 0) {
		TS_LOG_ERR("%s: create_and_get_u16_array err\n", __func__);
		goto err_get_u16_array;
	}
	return;

err_get_u16_array:
short_test_unsupport:
	chip_data->support_s3320_short_test = false;
}

static void synap_parse_crc_err_reset(struct ts_kit_device_data *chip_data, struct device_node *device)
{
	int retval = 0;
	...
	retval = of_property_read_u32(device, CRC_ERR_DO_RESET, &chip_data->support_crc_err_do_reset);
	if(retval < 0) {
		TS_LOG_ERR("%s: get crc_err_do_reset err, set default 0\n", __func__);
		chip_data->support_crc_err_do_reset = 0;
	}
	TS_LOG_INFO("%s: set crc_err_do_reset: %d\n", __func__, chip_data->support_crc_err_do_reset);

	return;
}

static void synap_parse_chip_tddi_specific_dts(struct ts_kit_device_data *chip_data, struct device_node *device)
{
	int retval = 0;
	int read_val = 0;
	...
	retval = of_property_read_u32(device, CSVFILE_USE_PRODUCT_SYSTEM_TYPE, &chip_data->csvfile_use_product_system);
	if (retval) {
	...
	retval = of_property_read_u32(device, TD43XX_EE_SHORT_TEST, &chip_data->td43xx_ee_short_test_support);
	if (retval) {
		TS_LOG_INFO("td43xx_ee_short_test_support is 0.\n");
		chip_data->td43xx_ee_short_test_support = 0;
		retval = 0;
	} else {
		TS_LOG_INFO("td43xx_ee_short_test_support = %d\n", chip_data->td43xx_ee_short_test_support);
	}

	retval = of_property_read_u32(device, CSVFILE_TRX_DELTA_TEST, &chip_data->trx_delta_test_support);
	if (retval) {
	...
	retval = of_property_read_u32(device, TEST_CAPACITANCE_VIA_CSVFILE, &chip_data->test_capacitance_via_csvfile);
	if (retval) {
	...
	if (!retval) {
		TS_LOG_INFO("get chip aft_data_addr = 0x%04x\n",
			    read_val);
		rmi4_data->synaptics_chip_data->aft_data_addr = (u16) read_val;
	} else {
		rmi4_data->synaptics_chip_data->aft_data_addr = 0;
		rmi4_data->synaptics_chip_data->support_aft = 0;
		TS_LOG_INFO("cannot get chip aft_data_addr, set support_aft disable\n");
	}
}

static void synap_parse_touch_switch_reg(struct ts_kit_device_data *chip_data, struct device_node *device)
{
	int retval = 0;
	int read_val = 0;
	...
	retval = of_property_read_u32(device, "touch_switch_reg", &read_val);
	if (retval) {
		TS_LOG_INFO("device touch_switch_reg not exit,use default value.\n");
		chip_data->touch_switch_reg = 0;
	}else{
		chip_data->touch_switch_reg = read_val;
		TS_LOG_INFO("get device touch_switch_reg:%02x\n", chip_data->touch_switch_reg);
	}

	retval = of_property_read_u32(device, "touch_switch_hold_off_reg", &read_val);
	if (retval) {
		TS_LOG_INFO("device touch_switch_hold_off_reg not exit,use default value.\n");
		chip_data->touch_switch_hold_off_reg = 0;
	}else{
		chip_data->touch_switch_hold_off_reg = read_val;
		TS_LOG_INFO("get device touch_switch_hold_off_reg:%02x\n", chip_data->touch_switch_hold_off_reg);
	}

	retval = of_property_read_u32(device, "touch_game_reg", &read_val);
	if (retval) {
		TS_LOG_INFO("device touch_game_reg not exit,use default value.\n");
		chip_data->touch_game_reg = 0;
	}else{
		chip_data->touch_game_reg = read_val;
		TS_LOG_INFO("get device touch_game_reg:%02x\n", chip_data->touch_game_reg);
	}

	retval = of_property_read_u32(device, "game_control_bit", &read_val);
	if (retval) {
		TS_LOG_INFO("device game_control_bit not exit,use default value.\n");
		chip_data->game_control_bit = 0;
	}else{
		chip_data->game_control_bit = (u8) read_val;
		TS_LOG_INFO("get chip specific game_control_bit = %d\n", chip_data->game_control_bit);
	}
}
 ...
 void  synap_parse_chip_specific_dts(struct ts_kit_device_data *chip_data)
{
	...
	if (!retval) {
		TS_LOG_INFO("get chip specific charger_supported = %d\n",
			    read_val);
		rmi4_data->synaptics_chip_data->ts_platform_data->feature_info.charger_info.charger_supported =
		    (u8) read_val;
	}
	...
	if (!retval) {
		TS_LOG_INFO("get chip specific glove_switch_bit = %d\n",
			    read_val);
		rmi4_data->synaptics_chip_data->ts_platform_data->feature_info.glove_info.glove_switch_bit =
		    (u16) read_val;
	}
	...
	if (!retval) {
		TS_LOG_INFO("get chip specific glove_switch_addr = 0x%04x\n",
			    read_val);
		rmi4_data->synaptics_chip_data->ts_platform_data->feature_info.glove_info.glove_switch_addr =
		    (u16) read_val;
	}
	...
	if (!retval) {
		...
		rmi4_data->synaptics_chip_data->ts_platform_data->feature_info.roi_info.roi_supported = (u8) read_val;
	} else {
		TS_LOG_INFO("can not get roi_supported value\n");
		rmi4_data->synaptics_chip_data->ts_platform_data->feature_info.roi_info.roi_supported = 0;
	}
	...
	if (!retval) {
		TS_LOG_INFO("get chip specific roi_control_addr = 0x%04x\n",
			    read_val);
		rmi4_data->synaptics_chip_data->ts_platform_data->feature_info.roi_info.roi_control_addr =
		    (u16) read_val;
	}
	...
	if (!retval) {
		TS_LOG_INFO("get chip specific roi_control_bit = %d\n",
			    read_val);
		rmi4_data->synaptics_chip_data->ts_platform_data->feature_info.roi_info.roi_control_bit = (u8) read_val;
	}
	...
	if (!retval) {
		TS_LOG_INFO("get chip specific roi_data_addr = 0x%04x\n",
			    read_val);
		rmi4_data->synaptics_chip_data->ts_platform_data->feature_info.roi_info.roi_data_addr = (u16) read_val;
	}
	...
	retval = of_property_read_u32(device, "ic_status_reg", &chip_data->ic_status_reg);
	if (retval) {
		TS_LOG_INFO("device ic_status_reg not exit,use default value 0.\n");
		chip_data->ic_status_reg = 0;
		retval = 0;
	}else{
		TS_LOG_INFO("get device ic_status_reg:0x%02x\n", chip_data->ic_status_reg);
	}
	...
	retval = of_property_read_u32(device, SYNAPTICS_SELF_CAP_TEST, &chip_data->self_cap_test);
	if (retval) {
		TS_LOG_INFO("device self_cap_test not exit,use default value.\n");
	...
	retval = of_property_read_u32(device,BLANC501100_TIANMA_FLAG, &chip_data->blanc501100_tianma_flag);
	if (retval) {
		TS_LOG_ERR("blanc501100_tianma_flag not exit,use default value 0");
		chip_data->blanc501100_tianma_flag = 0;
		retval = 0;
	}
			       ...
			       struct ts_kit_device_data *chip_data)
{
		...
		if (array_len <= 0 || array_len > RAWDATA_NUM) {
			TS_LOG_ERR
			    ("raw_data_limit length invaild or dts number is larger than:%d\n",
			     array_len);
			synaptics_raw_data_limit_flag = true;
		}
	...
	retval =
	    of_property_read_u32(device, "byte_to_byte_cmp_config_id",
				 &chip_data->byte_to_byte_cmp_config_id);
	if (retval) {
		chip_data->byte_to_byte_cmp_config_id = 0;
		TS_LOG_INFO("device byte_to_byte_cmp_config_id not found.\n");
	}

	retval =
	    of_property_read_u32(device, "delay_for_erase_fw",
				 &chip_data->delay_for_erase_fw);
	if (retval) {
	...
	retval = of_property_read_u32(device, SYNAPTICS_CHECK_BULCKED, &chip_data->check_bulcked);
	if (retval) {
		TS_LOG_INFO("device check bulcked not exist,use default value.\n");
		chip_data->check_bulcked = false;
		retval = 0;
	}else{
		TS_LOG_INFO("get device check_bulcked:%02x\n", chip_data->check_bulcked);
	}

	retval = of_property_read_u32(device, "support_aft", &value);
	if (retval) {
		chip_data->support_aft = 0;
		TS_LOG_INFO("device not support aft.\n");
	} else {
		chip_data->support_aft = value;
...
void ts_kit_power_gpio_enable(void)
{
	if (rmi4_data == NULL) {
...
EXPORT_SYMBOL(ts_kit_power_gpio_enable);

void ts_kit_power_gpio_disable(void)
{
	if (rmi4_data == NULL) {
		TS_LOG_ERR("rmi4_data == NULL\n");
		return;
...
EXPORT_SYMBOL(ts_kit_power_gpio_disable);
...
static int synaptics_chip_detect(struct ts_kit_platform_data *data)
{
	...
	synaptics_parse_dts(rmi4_data->synaptics_dev->dev.of_node,rmi4_data->synaptics_chip_data);
...
static int synaptics_init_chip(void)
{
	...
	lcd_huawei_ts_synaptics_register(&tpkit_synaptics_ops);
				   ...
				   unsigned short length)
{
	...
	if (retval != PAGE_SELECT_LEN) {
		if (rmi4_data->synaptics_chip_data->use_ub_supported) {
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F01:
				...
				if(SUPPORT_CRC_ERR_DO_RESET == rmi4_data->synaptics_chip_data->support_crc_err_do_reset) {
...
late_initcall(synaptics_ts_module_init);
